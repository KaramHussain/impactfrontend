<template>
  <div>
    ...
    <table class="table mb-sm claim-table">
      ...
      <template v-if="cptClaimDetails.length > 0">
        <!-- <tbody>
        ...
        <tbody>
          <tr v-for="(item, index) in cptClaimDetails" :key="index">
            
            <td>{{index + 1}}</td>
            <td>{{item?.place_of_service}}</td>
            <td>{{item?.code}}</td>
            <td></td>
            <td>
              <span v-for="(pointerItem, pointerIndex) in item.Diagnosis_code_pointer" :key="pointerIndex">
                {{pointerItem}}
              </span>
            </td>
            <td></td>
...
export default {
  ...
  mounted(){
    let cptcodeDetails = JSON.parse(localStorage.getItem('claimsDetails'))
    this.cptClaimDetails = cptcodeDetails.cptcode
    console.log('claimdetails line:::', cptcodeDetails)
  },
